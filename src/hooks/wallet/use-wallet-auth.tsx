...
import { usePlatformDetection, getExternalId } from "@/utils/platform";
...
async function sendOTP(args: sendOTP) {
  ...
  const response = await sphere.auth.sendOtp({
...
export default function useWalletAuth() {
  ...
  // Helper function to get the appropriate external ID
  const getAuthExternalId = (phoneNumber?: string) => {
    if (platformInfo.isTelegram && platformInfo.telegramUser?.id) {
      return platformInfo.telegramUser.id.toString();
    }

    if (platformInfo.isBrowser && phoneNumber) {
      return phoneNumber;
    }

    throw new Error(
      `Cannot determine external ID for platform: ${platformInfo.platform}`
    );
  };

  return {
    user: platformInfo.telegramUser,
    platform: platformInfo.platform,
    isTelegram: platformInfo.isTelegram,
    isBrowser: platformInfo.isBrowser,
    getAuthExternalId,
    signUpMutation,
    signInMutation,
    sendOTPMutation,
